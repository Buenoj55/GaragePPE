...
CREATE TRIGGER AjoutParticuliers
	...
	BEGIN 
		...
		IF nbC = 0 THEN 
			INSERT INTO CLIENTS VALUES(new.ID_CLIENT, new.NOM_CLIENT, new.MAIL_CLIENT, new.MDP_CLIENT, new.ADR_CLIENT, new.CP_CLIENT, new.VILLE_CLIENT, new.TEL_CLIENT, new.ETAT_CLIENT); 
		END IF; 
...
CREATE TRIGGER updateParticuliers
	...
	BEGIN
		UPDATE CLIENTS SET ID_CLIENT = new.ID_CLIENT, NOM_CLIENT = new.NOM_CLIENT, MAIL_CLIENT = new.MAIL_CLIENT, MDP_CLIENT = new.MDP_CLIENT, ADR_CLIENT = new.ADR_CLIENT, CP_CLIENT = new.CP_CLIENT, VILLE_CLIENT = new.VILLE_CLIENT, TEL_CLIENT = new.TEL_CLIENT, ETAT_CLIENT = new.ETAT_CLIENT WHERE ID_CLIENT = old.ID_CLIENT;
END //
Delimiter ;
...
DROP TRIGGER IF EXISTS deleteRDV; 
Delimiter // 
CREATE TRIGGER deleteRDV
	...
	BEGIN
		DELETE FROM RDV WHERE ID_VEHICULE = OLD.ID_VEHICULE; 
		DELETE FROM DEVIS WHERE ID_VEHICULE = OLD.ID_VEHICULE; 
END // 
DELIMITER ;

DROP TRIGGER IF EXISTS dateDevis; 
Delimiter // 
CREATE TRIGGER dateDevis
	BEFORE INSERT 
	ON DEVIS
	FOR EACH ROW
...
/*DROP TRIGGER IF EXISTS calculDevis; 
DELIMITER // 
CREATE TRIGGER calculDevis
	AFTER INSERT
	ON DEVIS
	FOR EACH ROW 
	...
	BEGIN
		DECLARE nbD int;
			DECLARE prixP double;
			DECLARE nbP int;

		SELECT COUNT(*) INTO nbD FROM DEVIS WHERE NUM_DEVIS = new.NUM_DEVIS;

		IF nbD = 0 THEN

			SELECT PRIX_PIECE INTO prixP FROM PIECES p, COMPOSER c, DEVIS d WHERE p.ID_PIECE = c.ID_PIECE AND c.ID_OPERATION = d.ID_OPERATION AND d.NUM_DEVIS = new.NUM_DEVIS;
